...
//----------------------------------------------------------------------------------------------------------------------
  ...
  void cedar::aux::gui::VtkSurfacePlot::init()
  {
    ...
    mpTransform->Scale(1.0,1.0,1.0);
    mpTransformFilter->SetInputConnection(mpPlane->GetOutputPort());
    mpTransformFilter->SetTransform(mpTransform);
    ...
    // Setup the view
    mpActor = vtkSmartPointer<vtkActor>::New();
    ...
    mpAxes = vtkSmartPointer<vtkCubeAxesActor2D>::New();
    mpAxes->SetInput(mpWarper->GetOutput());
    mpAxes->SetFontFactor(2.0);
    //axes shouldn't jump around
    mpAxes->SetInertia(0);
    mpAxes->SetFlyModeToNone();
    mpAxes->SetCamera(mpRenderer->GetActiveCamera());
    mpRenderer->AddViewProp(mpAxes);
  ...
  void cedar::aux::gui::VtkSurfacePlot::init()
  {
    ...
    for (int i = 0; i < 10; ++i)
    {
      ...
      mpTable->AddColumn(arr.GetPointer());
    }
    mpTable->SetNumberOfRows(10);
    ...
    for(int i=0; i<10; i++)
    {
      for(int j=0; j<10; j++)
      {
        mpTable->SetValue(i, j, 0.0);
      }
    }
    // Set up the surface plot we wish to visualize and add it to the chart.
    mpSurface->SetXRange(0, 9);
    mpSurface->SetYRange(0, 9);
