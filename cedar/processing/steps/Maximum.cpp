...
// CEDAR INCLUDES
#include "cedar/processing/steps/Maximum.h"
...
namespace
{
  bool declare()
  {
    ...
    (
      new ElementDeclarationTemplate<cedar::proc::steps::Maximum>
      (
        "Algebra",
...
//----------------------------------------------------------------------------------------------------------------------
cedar::proc::steps::Maximum::Maximum()
:
// outputs
mOutput(new cedar::aux::MatData(cv::Mat::zeros(1, 1, CV_32F)))
{
  ...
  this->declareOutput("maximum", this->mOutput);
...
void cedar::proc::steps::Maximum::sumSlot(cedar::proc::ExternalDataPtr slot, cv::Mat& sum, bool lock)
{
  cv::Mat tmp;
  ...
  for (size_t i = 0; i < slot->getDataCount(); ++i)
  {
    ...
    if (mat_data)
    {
      ...
      {
        tmp= cv::max( tmp.clone(), input_mat );
      }
...
void cedar::proc::steps::Maximum::compute(const cedar::proc::Arguments&)
{
  cedar::proc::steps::Maximum::sumSlot(this->mInputs, this->mOutput->getData(), false);
}

void cedar::proc::steps::Maximum::inputConnectionChanged(const std::string& /*inputName*/)
{
  if (this->mInputs->getDataCount() > 0)
  {
    ...
    if (changed)
    {
      this->emitOutputPropertiesChangedSignal("maximum");
    }
  }
}
