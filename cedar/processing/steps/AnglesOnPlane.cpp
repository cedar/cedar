...
namespace
{
  bool declare()
  {
    ...
    (
      ...
      "input vectors (see below) and returns the angle between them as well as "
      "other useful information.\n\n"
      "The plane is specified by the two input vectors 'vector a' and "
      "'vector b', which can be 3D. "
      "The output 'angle ab' is calculated between these two base vectors.\n"
      "USAGE: This is typically used for finding a plane for target "
      "acquisition. "
      "Use current velocity vector as 'vector a' and "
      "the difference between the current position and the target position "
      "'vector b' to span the plane."
      "This step will also return the angle between these two input vectors, "
      "as well as "
