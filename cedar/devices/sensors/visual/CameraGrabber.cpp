...
//mCreateFromConfigFile(false)
{
  ...
  mCameraCapabilitiesFileNames.push_back(useAutogeneratedFilenameString());
  ...
  std::stringstream s;
    s << "Camera channel 0"
      << ": DeviceID: " << mCamIds.at(0).busId
      << ", GUID: " << mCamIds.at(0).guid
      << " (0x"<<std::hex<<mCamIds.at(0).guid<<")"<<std::dec
      << ", Mode: " << CameraVideoMode::type().get(this->getCameraMode(0)).name();
  mChannelInfo.push_back(s.str());
...
cedar::dev::sensors::visual::GrabberInterface(configFileName)
{
  ...
  //the lock for concurrent access to the cv::VideoCapture
  QReadWriteLock *p_video_capture_lock_0 = new QReadWriteLock();
  mVideoCaptureLocks.push_back(p_video_capture_lock_0);
  QReadWriteLock *p_video_capture_lock_1 = new QReadWriteLock();
...
{
  doCleanUp();
  //rest is done by shared_pointer:
  // mCamCapabilities, mVideoCaptureLocks, ...
...
bool cedar::dev::sensors::visual::CameraGrabber::onDeclareParameters()
{
  ...
  for (unsigned int channel=0; channel<mNumCams; channel++)
  {
    ...
    result = 
      (addParameter(&mCameraCapabilitiesFileNames.at(channel),prop_name,prop_default)== CONFIG_SUCCESS) && result;
  }
  return result;
}
  ...
  for (unsigned int channel = 0; channel < mNumCams; ++channel)
   {
    ...
    if (!mCreateGrabberByGuid)
    {
      mCamIds.at(channel).guid = 
        static_cast<unsigned int>(mVideoCaptures.at(channel).get(CV_CAP_PROP_GUID));
    }
    ...
    unsigned int guid = mCamIds.at(channel).guid;
...
const std::string& cedar::dev::sensors::visual::CameraGrabber::onGetSourceInfo(unsigned int channel) const
{
  ...
  return mChannelInfo.at(channel);
}
