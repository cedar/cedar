...
//mCreateFromConfigFile(false)
{
  ...
  mCameraCapabilitiesFileNames.push_back(CAMERAGRABBER_CAM_CAPABILITIES_USE_AUTOGENERATED_FILENAME);
  ...
  #ifdef DEBUG_CAMERAGRABBER
    std::cout << "[CameraGrabber::CameraGrabber] Update configuration file" << std::endl;
  #endif
...
bool CameraGrabber::onDeclareParameters()
{
  ...
  for (unsigned int channel=0; channel<mNumCams; channel++)
  {
    ...
    std::string prop_default = CAMERAGRABBER_CAM_CAPABILITIES_USE_AUTOGENERATED_FILENAME;
    result = 
  ...
  for (unsigned int channel = 0; channel < mNumCams; ++channel)
   {
    ...
    if (cap_file_name == CAMERAGRABBER_CAM_CAPABILITIES_USE_AUTOGENERATED_FILENAME)
    {
      if (guid != 0)
      {
        std::string guid_str = boost::lexical_cast<std::string>(guid);
        size_t guid_found = cap_file_name.find(guid_str);
        if (guid_found == string::npos)
        {
          cap_file_name = CAMERAGRABBER_CAM_CAPABILITIES_FILENAME(guid);
        }
      }
      else
      {
        cap_file_name = CAMERAGRABBER_CAM_CAPABILITIES_FILENAME(channel);
      }
    }
