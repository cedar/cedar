...
Download the source code from our
[repository](https://bitbucket.org/cedar/dependencies/downloads/qwtplot3d-0.3.0-patched.tar.gz)
then open a terminal and go to the folder containing the downloaded file.
...
compiler open *XCode*, go to *Preferences* -> *Downloads* and install the
*Command Line Tools*. When configuring cedar with cmake change the compiler to
GCC. You can do this by using *ccmake*: Switch to advanced mode and then set
the value of `CMAKE_CXX_COMPILER` to `/usr/bin/llvm-g++`.
